#!/bin/bash
# Reset SuiteCRM Instance 2 database and environment to clean state
...
echo "=========================================="
echo "Resetting SuiteCRM Instance 2 (Port 8081)"
echo "=========================================="
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    if docker exec suitecrm_setup_2-mariadb2-1 mysqladmin ping -h localhost --silent 2>/dev/null; then
...
while [ $RETRY_COUNT -lt $MAX_RETRIES ]; do
    ...
    USERS_EXISTS=$(docker exec suitecrm_setup_2-mariadb2-1 mysql -u bn_suitecrm -pbitnami123 bitnami_suitecrm -e "SHOW TABLES LIKE 'users';" 2>/dev/null | grep -c "users" || echo "0")
    CONTACTS_EXISTS=$(docker exec suitecrm_setup_2-mariadb2-1 mysql -u bn_suitecrm -pbitnami123 bitnami_suitecrm -e "SHOW TABLES LIKE 'contacts';" 2>/dev/null | grep -c "contacts" || echo "0")
    ACCOUNTS_EXISTS=$(docker exec suitecrm_setup_2-mariadb2-1 mysql -u bn_suitecrm -pbitnami123 bitnami_suitecrm -e "SHOW TABLES LIKE 'accounts';" 2>/dev/null | grep -c "accounts" || echo "0")
    EMAIL_REL_EXISTS=$(docker exec suitecrm_setup_2-mariadb2-1 mysql -u bn_suitecrm -pbitnami123 bitnami_suitecrm -e "SHOW TABLES LIKE 'email_addr_bean_rel';" 2>/dev/null | grep -c "email_addr_bean_rel" || echo "0")
...
if [ $RETRY_COUNT -eq $MAX_RETRIES ]; then
    ...
    echo "You may need to access http://localhost:8081 first to complete SuiteCRM setup."
    echo "After SuiteCRM is fully initialized, you can manually load demo data with:"
    echo "  docker exec -i suitecrm_setup_2-mariadb2-1 mysql -u bn_suitecrm -pbitnami123 bitnami_suitecrm < init-db/demo_data.sql"
    exit 0
fi
...
echo "=========================================="
echo ""
echo "SuiteCRM Instance 2 is available at: http://localhost:8081"
